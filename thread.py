...
if __name__ == "__main__":
    httpd = HTTPServer(('localhost', 8080), MyRequestHandler) # create the server
    threading.Thread(target=serve_forever, args=(httpd,)).start() # start the server in a thread, so it doesn't block the main thread
